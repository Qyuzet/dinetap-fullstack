  ...
  try {
    ...
    try {
      ...
      const restaurantName = await page.evaluate(() => {
        ...
        const logoImg = document.querySelector('header img, .logo img, #logo img');
      ...
      const menuItems = await page.evaluate(() => {
        ...
        possibleMenuElements.forEach(el => {
          ...
          const nameEl = el.querySelector('h3, h4, h5, .name, .title, [class*="name"], [class*="title"]');
          const priceEl = el.querySelector('.price, [class*="price"]');
          const descEl = el.querySelector('.description, [class*="description"], p');
        ...
        if (items.length === 0) {
          ...
          const priceElements = Array.from(allElements).filter(el => {
            ...
            return priceRegex.test(text) && text.length < 100 && !el.querySelector('*');
          });
...
${menuItems.map(item => `- ${item.name}: ${item.description || "No description"} - Price: ${item.price || "Unknown"}`).join('\n')}
